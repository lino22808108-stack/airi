...
describe('streamFrom tool errors', () => {
  ...
  it('emits the final xsAI messages after all tool rounds finish', async () => {
    ...
    await streamFrom({
      ...
      conversation: { turns: readChatTurns(finalMessages.slice(0, 1)) },
      options: { onTranscript },
    })

    expect(onTranscript).toHaveBeenCalledTimes(1)
    expect(onTranscript.mock.calls[0][0].rounds).toHaveLength(2)
    expect(renderChatMessages({ turns: [onTranscript.mock.calls[0][0]] })).toEqual(finalMessages.slice(1))
  })
  ...
  it('ignores provider errors after steps resolve while final messages are pending', async () => {
    ...
    const pending = streamFrom({
      model: 'model-a',
      chatProvider: provider,
      conversation: { turns: readChatTurns([{ role: 'user', content: 'hello' }]) },
    })
  ...
  it('maps xsai tool-error results to AIRI tool-error events without wrapping tools', async () => {
    ...
    await streamFrom({
      model: 'model-a',
      chatProvider: provider,
      conversation: { turns: readChatTurns([{ role: 'user', content: 'play chess' }]) },
      options: {
        tools: [failingTool],
        onStreamEvent: (event) => {
          events.push(event)
...
describe('chat protocol compatibility', () => {
  function projectChat(messages: Parameters<typeof readChatTurns>[0], supportsContentArray = true) {
    return renderChatMessages({ turns: readChatTurns(messages) }, supportsContentArray)
  }
