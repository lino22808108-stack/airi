...
import { renderChatMessages } from './chat-completions'
...
it('preserves nested provider extensions on native Chat replay', () => {
  ...
  const result = renderChatMessages({ turns: [{ type: 'assistant', id: 'turn', status: 'completed', rounds: [{ id: 'round', content: [], toolInvocations: [], projectionIssues: [], continuation: { protocol: 'chat-completions', scope: 'owner', data: native } }] }] }, true, 'owner')
