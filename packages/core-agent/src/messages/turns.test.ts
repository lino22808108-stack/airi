...
import { readChatTurns, renderChatMessages } from './chat-completions'
...
it('keeps application context below trusted system and developer instructions', () => {
  ...
  expect(renderChatMessages(conversation)).toEqual([
...
it('scopes imported rounds and tool invocations by the stored turn identity', () => {
  ...
  const [first] = readChatTurns(history, 'stored-first')
  const [second] = readChatTurns(history, 'stored-second')
