 ...
 *
 * @example
 * readChatTurns([{ role: 'user', content: 'Hello' }])[0].segments
 * // => [{ type: 'text', text: 'Hello' }]
 */
export function readChatEntries(messages: (ChatMessage | { role: 'error', content: string })[], idPrefix = 'message'): ProjectionEntry[] {
...
export function readChatTurns(messages: Parameters<typeof readChatEntries>[0], idPrefix?: string): Turn[] {
  return readTurns(readChatEntries(messages, idPrefix))
}
 ...
 */
export function renderChatMessages(conversation: Conversation, supportsContentArray = true, scope?: string): ChatMessage[] {
  // NOTICE:
